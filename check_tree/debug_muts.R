...
save(subs,file = "subs")
save(outputs,file = "outputs")

setwd(paste0(begin_path,"vars/debug_muts"))
load("subs")
setwd(paste0(begin_path,"vars/"))
load("site_details")
coding_sites = regions$ref_site[which(regions$gene!=0)]
coding_subs = subs[which(is.element(subs$ref_site,coding_sites)),]
edges = apply(coding_subs[,c("parent","base")],1,paste0,collapse = ".")
unique_edges = unique(edges)
adjacent_coding_subs_edges = NULL
for (i in 1:length(unique_edges)){
  plcs = which(edges==unique_edges[i])
  print(i)
  if (length(plcs)>1){
    sorted_ref_sites = sort(as.numeric(coding_subs$ref_site[plcs]))
    if(sum(diff(sorted_ref_sites)==1)>0){
      adjacent_coding_subs_edges = c(adjacent_coding_subs_edges,unique_edges[i])
    }
  }
}

for(i in 1:length(adjacent_coding_subs_edges)){
  plcs = which(edges==adjacent_coding_subs_edges[i])
  sorted_ref_sites = sort(as.numeric(coding_subs$ref_site[plcs]))
  print(sorted_ref_sites)
}