...
#########################################################
# The ASR for each site takes about 5 minutes for the full tree described in the paper. 
# I recommend dividing the unique_site_patterns and corresponding site_patterns_numbers into groups according to the available number of cores. 
...
setwd(paste0(begin_path,"/vars/"))
load("codons_table")
datasets_preprocess(codons_table,testing_flag = 0)
...
# parallel_vals = expand.grid(66,23,6,5) # This is a subset of models, uncomment this line if you
# # want to get ~fast partial results (still takes a couple of hours).
...
tmp = create_codons_table(codon_states = codon_states,
                          ...
                          testing_flag = 1)
codons_table_testing = create_codons_table_testing(codons_table = tmp,new_subs_testing,
                                                   rel_ref_sites_no_subs,
...
# Lift results for all examined models - the first 2 models are null models and the rest are 
# the top models evaluated (their number is determined by "num_first_models" in the previous section)
source(paste0(begin_path,"analyse_predictions.R"))
analyse_predcitions_preprocess(begin_path)

setwd(paste0(begin_path,"/vars/"))
load("codons_table_amino_diff_output")
load("amino_predictions_diff_output")
load("testing_plcs")
load("prediction_names")
load("num_models")
examined_model_num = 1 # The number is the rank of the model in the sorted unique_models list.
...
tmp =AUC_and_Lift_plot(spike_flag,codons_table_amino_diff_output,
