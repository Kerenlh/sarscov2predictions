...
#     name_split = strsplit(file_names[i],split = "[.]")[[1]]
#     if (name_split[1]=="short_details"){
#       nums = c(nums,strsplit(name_split[3],split = "[_]")[[1]][1])
#     }
#   }
# }
...
#   in_progress = NULL
# }
# nums = c(nums,in_progress)
# nums = as.numeric(nums)
# rel_nums = c(1:dim(parallel_vals)[1])
# missing_nums = rel_nums[which(is.element(rel_nums,nums)==FALSE)]
# # if(cluster){
# #   rel_nums = c(1:dim(parallel_vals)[1])
# #   missing_nums = rel_nums[which(is.element(rel_nums,nums)==FALSE)]
# # }else{
# #   rel_nums = c(4691:dim(parallel_vals)[1])
# #   missing_nums = rel_nums[which(is.element(rel_nums,nums)==FALSE)]
# # }
# 
# print(missing_nums[1:min(10,length(missing_nums))])
# print(paste("length(missing_nums):",length(missing_nums)))
# parallel_num = missing_nums[sample(1:length(missing_nums),1)]
# 
# in_progress = c(in_progress,parallel_num)
# save(in_progress,file = "in_progress")
# print(parallel_num)
# 
...
create_details = function(parallel_num){
  print(parallel_num)
  # begin_path = "/Users/keren/Dropbox/covid/new2/"
  begin_path = "/home/ubuntu/Dropbox/covid_Saharon_AWS/new2/"
  parallel_vals = expand.grid(1:66,1:23,1:6,1:5)
  colnames(parallel_vals) = c("codon","amino_acid","base","codon.pos")
  plcs = which(parallel_vals[,"codon"]<66 & parallel_vals[,"amino_acid"]!=23)
  parallel_vals = parallel_vals[-plcs,]
  plcs = which(parallel_vals[,"codon"]<66 & parallel_vals[,"codon.pos"]<5 &
  ...
  is_neighbor_same_as_codon = function(curr_data,rm_cols){
  ...
  colnames_short_details =c(input_iterate,"output","reg_name2","count",
                            "u","t","w","q","a","p","k","j","b")
  # short_details = matrix(0,500000,length(colnames_short_details))
  short_details = NULL
  ...
  b = parallel_vals[parallel_num,"base"]
  j = parallel_vals[parallel_num,"codon.pos"]
  
  
  data_rows = matrix(0,dim(data)[1],1)
  rownames(data_rows) = rownames(data)
  for (i in 1:9){#length(output)){
                    ...
                    for (u in u_max:iterate_vals[1,"L.neighbor"]){
                      ...
                      for (m in 1:iterate_vals[1,"gene"]){
                        ...
                        if (dim(curr_data)[1]!=0){
                          ...
                          rm_cols = is_neighbor_same_as_codon(curr_data,rm_cols)
                          ...
                          if (is.na(dim(curr_data)[1])==FALSE){
                            ...
                            curr_short_details = cbind(selected_input,i,reg_name2,count,
                                                       u,t,w,q,a,p,k,j,b)
                            # if (i>2 & i<6){ # transitions transversions and y not for A/C/G/T because it contains different rows
                            #   curr_output = selected_output
                            #   remaining_outputs = output[3:5]
                            #   remaining_outputs = 
                            #     remaining_outputs[-which(remaining_outputs==curr_output)]
                            #   while(length(remaining_outputs)>0){
                            #     same_cols = NULL
                            #     for (c in 1:length(remaining_outputs)){
                            #       if (identical(data[data_rows,curr_output],
                            #                     data[data_rows,remaining_outputs[c]])){
                            #         same_cols = c(same_cols,c)
                            #         count = count+1
                            #         new_output = which(output==remaining_outputs[c])
                            #         tmp = c(selected_input,new_output,reg_name2,count,
                            #                 u,t,w,q,a,p,k,j,b)
                            #         curr_short_details = rbind(curr_short_details,tmp)
                            #       }
                            #     }
                            #     if (length(same_cols)>0){
                            #       remaining_outputs = remaining_outputs[-same_cols]
                            #     }
                            #     if (length(remaining_outputs)>0){
                            #       count = count+1
                            #       curr_output = remaining_outputs[1]
                            #       remaining_outputs = remaining_outputs[-1]
                            #       reg_name = paste0(c(paste0(data_rows,collapse = ""),
                            #                           paste0(c(paste0(colnames(curr_data)
                            #                                           [1:(length(colnames(curr_data))-1)],collapse = "."),
                            #                                    curr_output),collapse = ".")),
                            #                         collapse = ".")
                            #       reg_name2 = digest(reg_name,algo = "md5")
                            #       new_output = which(output==curr_output)
                            #       tmp = c(selected_input,new_output,reg_name2,count,
                            #               u,t,w,q,a,p,k,j,b)
                            #       curr_short_details = rbind(curr_short_details,tmp)
                            #     }
  ...
  while(file.exists(file_name)==FALSE){
    ...
    print(parallel_num)
    print(paste("save_count  ",save_count))
    save(short_details,file = file_name)
    if (file.exists(file_name)){
      print("Good, saved!")
      parallel_nums = parallel_nums[-parallel_nums_plc]
      save(parallel_nums,file = "parallel_nums")
      print(parallel_nums)
      print(paste("length(parallel_nums):",length(parallel_nums)))
    }else{
      print("Not saved!")
    }
  }
  # load("in_progress")
  # in_progress = in_progress[-which(in_progress==parallel_num)]
  # save(in_progress,file = "in_progress")
...
setwd(paste0(begin_path,"vars/datasets/"))
load("missing_nums")
parallel_nums = missing_nums # length(unique_dataset)/iter_num
print(Sys.time())
