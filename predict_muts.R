...
#   plcs = which(codons_table_prediction$ref_site==all_sites[i])
#   codons_table_prediction$exposure[plcs] = 
#     100*codons_table_prediction$exposure[plcs]/sum(codons_table_prediction$exposure[plcs])
# }
...
tmp = unique_models[which(unique_models$output==1),c("gene","base","amino_acid","codon","codon.pos","mat_peptide",
                               "stem_loop","CG","R.neighbor","L.neighbor",
                               "total_num_models","AIC.NB","AIC.P")]
setwd("/Users/keren/Dropbox/covid/new2/figures/")
write.csv(tmp,file = "Lanfear_models.csv")
...
model_nums = c(null_model_num,null_time,c(1:4))
# model_nums = c(null_model_num,null_time,c(454,6,12069,12181,12396,12,632,1325,347,933)) # Poisson winning models
# model_nums = c(null_model_num,null_time,2,5,1,4,12,12069,12,12181,12396,6)
# model_nums = c(null_model_num,null_time,2,5,1,4,12,12069,12181,12396,6)
...
######################
setwd(paste0(curr_begin_path,"vars/prediction/"))
table(codons_table_prediction$diff_seqs_num)
unique_models[1,]
if(diff_seqs_num_flag==1){
  save(predictions,file = "no_sites_normalization.predictions_syn_non_syn.more_diff_seqs")
  save(internal_predictions,file = "no_sites_normalization.internal_predictions_syn_non_syn.more_diff_seqs")
  save(debug.nb.converged,file = "no_sites_normalization.debug.nb.converged.syn_non_syn.more_diff_seqs")
  save(debug.p.converged,file = "no_sites_normalization.debug.p.converged.syn_non_syn.more_diff_seqs")
}else if (diff_seqs_num_flag==0){
  save(predictions,file = "predictions_syn_non_syn.1_diff_seq")
  save(internal_predictions,file = "internal_predictions_syn_non_syn.1_diff_seq")
  save(debug.nb.converged,file = "debug.nb.syn_non_syn.converged.1_diff_seq")
  save(debug.p.converged,file = "debug.p.converged.syn_non_syn.1_diff_seq")
}else if (diff_seqs_num_flag==2){
  save(predictions,file = "predictions_syn_non_syn.all_seqs")
  save(internal_predictions,file = "internal_predictions_syn_non_syn.all_diff_seqs")
  save(debug.nb.converged,file = "debug.nb.converged.syn_non_syn.all_diff_seqs")
  save(debug.p.converged,file = "debug.p.converged.syn_non_syn.all_diff_seqs")
}
