...
categorical_names = c("L.neighbor","R.neighbor","codon","codon.pos","gene",            
                      "stem_loop","amino_acid","base")


data$mat_peptide[which(data$mat_peptide!="0")] = 1
data$mat_peptide = as.numeric(data$mat_peptide)
data = add_contrasts(data,categorical_names)
...
get_data = function(curr_row,iterate_vals){
...
# syn_exposure = non_syn_exposure = matrix(0,dim(data)[1],1)
# data = cbind(data,syn_exposure,non_syn_exposure)
# load("details_non_codons")
# load("iterate_vals_non_codons")
# iterate_vals = iterate_vals_non_codons
...
# save(iterate_vals,file = "iterate_vals_codons")




# #Debug example
# plc =241205 # i is the relevant row number in details
# tmp = get_data(curr_row = details[plc,],iterate_vals = iterate_vals)
...
# i=1
# dataset[i,]
# unique(dataset$output)
# dataset$output = 1
# get_data(dataset[i,],iterate_vals = iterate_vals)
