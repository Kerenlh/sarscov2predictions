...
if(file.exists("all_parallel_nums")){
  load("all_parallel_nums")
}else{
  all_parallel_nums = c(1:100)
}
parallel_nums = all_parallel_nums[1]
all_parallel_nums = all_parallel_nums[-1]
save(all_parallel_nums,file = "all_parallel_nums")
####
...
for (p in 2:50){
  ...
  # # tmp = which(codon_pos_2_plcs[1:length(codon_pos_1_plcs)]-codon_pos_1_plcs!=1)
  # codon_pos_1_plcs[tmp[1]]
  # tmp = which(codon_pos_3_plcs-codon_pos_1_plcs!=2)
  # codon_pos_2_plcs[tmp[1]]
  # tmp = which(codon_pos_1_plcs[2:length(codon_pos_1_plcs)]-
  #               codon_pos_3_plcs[1:(length(codon_pos_3_plcs)-1)]!=1)
  # codon_pos_3_plcs[tmp]
