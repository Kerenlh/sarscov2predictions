...
get_plot_vars = function(spike_flag,codons_table_amino_diff_output,amino_predictions_diff_output){
...
divide_codons_table_to_outputs = function(codons_table,syn_ti_const,syn_tv_const,
...
#########################
analyse_predcitions = function(begin_path,spike_flag,examined_model_num,model_type){
  library("ggplot2")
  library("gtable")
  library("grid")
  library("gridExtra")
  ...
  codons_table_amino = get_codons_table_amino(testing_plcs,codons_table_testing_divided,divided_predictions)
  ...
  ratios = get_ratios(testing_plcs,curr_codons_table,syn_plcs,non_syn_plcs,
                      curr_predictions,spike_flag,num_models)
  results = get_results(testing_plcs,curr_codons_table,syn_plcs,non_syn_plcs,ratios,num_models)
  
  j = which(names(predictions)==model_type)
  
  if (spike_flag){
    tmp = get_plots(i=(examined_model_num+2),j,spike_flag=1,ratios,syn_plcs,non_syn_plcs,
                    curr_codons_table,curr_predictions)
