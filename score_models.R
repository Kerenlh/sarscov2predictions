...
find_id_plcs = function(model_ids,unique_models){
  ...
  cat("\n")
  pb <- txtProgressBar(min = 0, max = dim(unique_models)[1], style = 3, width = 50, char = "=")
...
bad_plcs = which(unique_models$codon.pos==0)
if (length(bad_plcs)>0){
  unique_models = unique_models[-bad_plcs,]
}
unique_models = unique_models[order(apply(unique_models[,c("AIC.NB","AIC.P")],1,min)),]
setwd(paste0(begin_path,"vars/"))
save(unique_models,file = "unique_models")
save(model_ids,file = "model_ids")
